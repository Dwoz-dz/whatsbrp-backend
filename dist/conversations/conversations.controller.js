...
let ConversationsController = class ConversationsController {
    ...
    async listMine(req) {
        ...
        if (!userId)
            throw new common_1.UnauthorizedException('Missing user in request');
        return this.convs.listMyConversations(userId);
    }
    async listMessages(req, conversationId, limit, cursor) {
        const userId = req.user?.id;
        if (!userId)
            throw new common_1.UnauthorizedException('Missing user in request');
...
], ConversationsController.prototype, "listMine", null);
__decorate([
